# How the video's Wispr Flow clone works
...
The video is a "build this with Claude Code" walkthrough, not a product teardown. The creator's claim is blunt. Wispr Flow "just raised $280 million at a $2 billion valuation for exactly what I'm doing right now using a tool I built in 20 minutes." The ask they type into Claude is also blunt. Clone Wispr Flow. Look it up. Make a push-to-talk dictation app. Recommend architecture for this machine. Start with a skeleton, then a real macOS app.

Claude's plan is the architecture we should steal at the pipeline level.
...
They name the app Murmur, then Murmur YouTube, because an older clone already occupied the Accessibility permission slot. They grant Microphone and Accessibility. The prototype transcribes. It does not type into Cursor. Claude's logs say injection succeeded. The creator says, "That's the classic AX silent failure." After the fix, it types into Cursor and a Google Doc. They add an engine comparison window, pick Apple as the default because it needs no model download, then grow a history window, a dictionary, and an 1980s tape-recorder look.

That is a Mac app. We need Linux too. The pipeline stays. The shell does not.

## Key concepts

**Wispr Flow the product.** [wisprflow.ai](https://wisprflow.ai/) sells "what you meant," not raw ASR. Their homepage demo takes a ramble full of ums, false starts, and "actually Monday" and emits a sendable paragraph. They also ship a personal dictionary, snippets, per-app tone, and 100+ languages. Flow types wherever the cursor is. Slack, Gmail, Notion, terminals, Cursor. Built-in dictation, they argue, writes every um and then leaves you to edit. Flow is not available on Linux.

Do not confuse that product with [whisperflow.org](https://whisperflow.org/), a different company's voice keyboard. The video is cloning Wispr Flow.
...
**Parakeet is the portable engine.** The video downloads NVIDIA Parakeet when Apple is not enough, and promises a separate repo for Windows users. Linux needs that portable engine on day one. [OpenWhispr](https://github.com/OpenWhispr/openwhispr) already runs Parakeet through sherpa-onnx and Whisper through whisper.cpp on Mac, Windows, and Linux. Their [local-models guide](https://docs.openwhispr.com/guides/local-models) lists `parakeet-tdt-0.6b-v3` at about 680 MB, INT8 ONNX, 25 languages, CPU-fine.

**Injection is the product.** Transcription that stays on the clipboard is a toy. The video learns this in Cursor. Open-source clones hit the same wall. [EnviousWispr PR 278](https://github.com/saurabhav88/EnviousWispr/pull/278) documents Chrome returning `kAXErrorNoValue` for the focused element, so a strict AX-only path falls through to clipboard-only. [VaulType's TEXT_INJECTION.md](https://github.com/vaultype/VaulType/blob/main/docs/features/TEXT_INJECTION.md) describes the working cascade. Try a real text field through Accessibility. Fall back to clipboard plus a synthetic paste. Restore the user's clipboard. Treat AX "success" as a rumor.

**Linux injection is a different rumor.** Wayland compositors do not give you `CGEventTap`. Tools that work today pick from libei / the emulated-input portal, `ydotool` over uinput, `wtype` on wlroots, `xdotool` on X11, and clipboard paste. [xhisper-rs](https://github.com/PrivateGER/xhisper-rs) tries Wayland Emulated Input, then clipboard, then uinput. [flowvoice](https://github.com/GOJO-SENPA1/flowvoice) auto-selects `wtype` / `ydotool` / `xdotool`. [whisrs](https://github.com/y0sif/whisrs) types through uinput plus XKB so layout-unaware keystrokes do not become garbage.

**Cleanup is Wispr's real moat.** The video says this out loud. Punctuation and um-stripping are optional, added "just to see how good it can actually get to Whisper Flow." [paretoimproved/murmur](https://github.com/paretoimproved/murmur) is the Linux version of that sentence. Raw Whisper is not the product. A local LLM pass is.
...
The video's comparison window records once and scores Apple vs Parakeet vs Wispr Flow. Their Wispr number is dirty. They "ping Whisperflow's database" for timing, so the clock includes IPC they do not own. They still conclude the gap is small and pick Apple for zero-setup. That pick is correct for a Mac-only demo on macOS 26. It is wrong as Echo's default. Linux has no SpeechAnalyzer. The shared default has to be Parakeet or whisper.cpp.
...
- Video architecture. Swift Murmur clones such as [janisbelozerovs-dev/murmur](https://github.com/janisbelozerovs-dev/murmur) and [daviddao/murmur](https://github.com/daviddao/murmur).
- Linux loop. [whisrs](https://github.com/y0sif/whisrs), [xhisper-rs](https://github.com/PrivateGER/xhisper-rs), [flowvoice](https://github.com/GOJO-SENPA1/flowvoice), [paretoimproved/murmur](https://github.com/paretoimproved/murmur).
- Portable engines. [OpenWhispr](https://github.com/OpenWhispr/openwhispr), [sherpa-onnx](https://github.com/k2-fsa/sherpa-onnx), [whisper.cpp](https://github.com/ggml-org/whisper.cpp).
...
**Do not start in Swift and port.** The video's own next step was "a separate GitHub repo" for non-Mac users. That split is how clones die. Rust from phase 1 is the whole point of this plan.
