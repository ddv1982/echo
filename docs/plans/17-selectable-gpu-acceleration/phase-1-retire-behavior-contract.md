...
**`crates/echo/tests/fixtures/`.** Delete `whisper-behavior-v3.json` and `whisper-v3-identities.json`. Both exist only to pin the projection and the identity algebra to it.
...
- `grep -r projectionSha256 crates/ .github/` returns nothing.
