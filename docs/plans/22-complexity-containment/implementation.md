...
The [settings collection report](settings-after.json) passed all ten cases. Its source hash matches the committed application source. The [final native probe](native-after.json) passed all ten recording/configuration contracts with no source change during the run. The current-status lane retained a 1 ms p95 under the same debug, fake-engine configuration. Individual timing samples are omitted from the committed native reports; their summaries and source/binary identities are retained.
