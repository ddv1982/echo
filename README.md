...
Managed downloads resume after cancellation or interruption, check free disk space before HTTP, verify SHA-256, extract only catalogued archive payloads, and activate one immutable component generation at a time. Echo hashes every managed file on the first check in each process and after its file identity or metadata changes. An explicit Verify always hashes every file. This detects persistent payload changes, but it does not defend against an active writer that runs under the same account as Echo. Repair replaces missing or corrupt generations. Remove deletes only files recorded under Echo's managed directory. System runtimes on `PATH` and models copied into the existing flat cache remain external inputs; Echo labels and uses them but never repairs or removes them.
