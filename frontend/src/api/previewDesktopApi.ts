...
import {
  PREVIEW_RECORDING_POLICY,
  PREVIEW_TRAINING_TRANSCRIPTS,
  defaultPreviewDevices,
  defaultPreviewDictionary,
  defaultPreviewGpuDevices,
  defaultPreviewHistory,
  defaultPreviewInventory,
  defaultPreviewLanguages,
  defaultPreviewMicrophones,
  defaultPreviewReadiness,
  richPreviewStatus,
} from './previewDesktopFixtures'
...
export function createPreviewDesktopApi(): PreviewDesktopApi {
  ...
  let previewHistory: HistoryItem[] = defaultPreviewHistory()
  ...
  function finishDictionaryTrainingSample(captureId: string): Promise<DictionaryTrainingSample> {
    ...
    const transcript = PREVIEW_TRAINING_TRANSCRIPTS[previewTrainingIndex % PREVIEW_TRAINING_TRANSCRIPTS.length]
